...
__all__ = ["PlanetaryModel2SPH", "convert_planetary_model_to_SPH", "pickle_planetary_model"]

PlanetaryModelInSPH = namedtuple('PlanetaryModelInSPH', ['gas_particles', 'core_particle', 'core_radius'])

class PlanetaryModel2SPH(object):
    """
    Requests the internal structure of the star from a Planetary Evolution
    ...
    :argument csv_file: If provided, read planetary structure from here instead of using 'particle'
    ...
    def __init__(self, particle, number_of_sph_particles, seed = None,
            ...
            with_core_particle = False, target_core_mass = None, csv_file = None,
            do_store_composition = True, gamma=1.72/0.72, base_grid_options = dict(type = "bcc")):
        ...
        self.core_radius = None # target_core_radius # set these variables?
        self.core_mass = None # target_core_mass # set these variables?
    ...
    def retrieve_planetary_structure(self):
    ...
    def setup_core_parameters(self):
        ...
        i_edge = np.searchsorted(interpolator.enclosed_mass.number, self.target_core_mass.value_in(interpolator.enclosed_mass.unit))-1
        min_i = i_edge
...
#~            entropy[:i_edge+1] = (entropy[:i_edge+1] + entropy[i_edge+1]) / 2.0
    ...
    def get_index(self, value, sorted_vector):
        ...
        index = np.searchsorted(sorted_vector, value)
        return max(index - 1, 0)

    def get_indices(self, values, sorted_vector):
        values = np.array(values.value_in(sorted_vector.unit))
        ...
        indices = np.maximum(np.searchsorted(sorted_vector, values) - 1, 0)
        return indices

    def calculate_interpolation_coefficients(self, radial_positions):
#        indices = np.array([self.get_index(r, self.midpoints_profile) for r in radial_positions])
    ...
    def relax(self, particles):
        ...
        for i in range(1, num_iterations+1):
            ...
            factor = np.minimum((max_delta * internal_energies / (accelerations.lengths() * smoothing_lengths)), 0.5)
            result.append(str(i) + ": Accelerations correlated: " + str(acc_correlated) + ", median factor: " + str(np.median(factor)))
    ...
    def result(self):
        ...
        #     self.retrieve_planetary_structure()
        # else:
        self.load_planetary_structure()
        ...
        if self.with_core_particle and self.core_radius:
            ...
            return PlanetaryModelInSPH(gas_particles=sph_particles, core_particle=core_particle, core_radius=self.core_radius)
        return PlanetaryModelInSPH(gas_particles=sph_particles, core_particle=None, core_radius=None)


def convert_planetary_model_to_sph(particle, number_of_sph_particles, **keyword_arguments):
    """
    Requests the internal structure of the planet from a csv file that
    contains a density profile, a radius profile and an internal energy
    profile and converts it into an SPH model consisting of the
    specified number of sph particles for the atmosphere, and a core particle.

    :argument particle: None
    :argument number_of_sph_particles: Number of gas particles (atmosphere) in the resulting model
    :argument with_core_particle: Model the core as a heavy, non-sph particle, set as True
    :argument target_core_mass:   If (with_core_particle): target mass for the non-sph particle
    :argument do_relax: Relax the SPH model - doesn't seem to work satisfactorily yet!
    :argument sph_code: set to Gadget2, can change that
    :argument csv_file: If provided, read planetary structure from here instead of using 'particle'
    :argument do_store_composition:  If set, store the local chemical composition on each particle, not enabled for now
    :argument gamma: adiabatic constant, for earth: gamma=1+(1/polytropy_index), where polytropic_index=0.72
    :argument base_grid_options: dict() with options for the initial distribution,
        see new_uniform_spherical_particle_distribution
    """
    converter = PlanetaryModel2SPH(particle, number_of_sph_particles, **keyword_arguments)
    return converter.result

# for compatibility
convert_planetary_model_to_SPH = convert_planetary_model_to_sph

def pickle_planetary_model(particle, csv_file_name):
    """
    Requests the internal structure of the star from a Planetary Evolution community
    code and pickles it (stores it as a *.pkl file), for later use:
    convert_planetary_model_to_sph(None, ..., csv_file=csv_file_name)
    Using a pickled planetary model is significantly faster for modelling giants
    and other extremely evolved stars.

    :argument particle: Star particle to be converted to an SPH model later
    :argument csv_file_name: Name of the pickle file in which to store the planetary structure
    """
    if os.path.isdir(os.path.dirname(os.path.abspath(csv_file_name))) and not os.path.exists(csv_file_name):
        outfile = open(csv_file_name, 'wb')
    else:
        raise AmuseWarning("Incorrect file name '{0}'; directory must exist and "
            "file may not exist".format(csv_file_name))
    converter = PlanetaryModel2SPH(particle, None)
    converter.retrieve_planetary_structure()
    pickle.dump(dict(
        mass                = converter.mass,
